...
export class HopLp extends ethereum.SmartContract {
  static bind(address: Address): HopLp {
    return new HopLp("HopLp", address);
  }
  ...
  swap(): Address {
    let result = super.call("swap", "swap():(address)", []);

    return result[0].toAddress();
  }

  try_swap(): ethereum.CallResult<Address> {
    let result = super.tryCall("swap", "swap():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
